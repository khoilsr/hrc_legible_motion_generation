 ...
 */ 
template<typename Scalar, int RowsAtCompileTime, int ColsAtCompileTime>
inline bool saveMatrixHrc(std::string directory, std::string filename, Eigen::Matrix<Scalar,RowsAtCompileTime,ColsAtCompileTime> matrix, bool overwrite)
{
  if (directory.empty())
    return false;
  ...
  filename = directory+"/"+filename;
  saveMatrixHrc(filename,matrix,overwrite);
 ...
 */ 
template<typename Scalar, int RowsAtCompileTime, int ColsAtCompileTime>
inline bool saveMatrixHrc(std::string filename, Eigen::Matrix<Scalar,RowsAtCompileTime,ColsAtCompileTime> matrix, bool overwrite)
{
  if (boost::filesystem::exists(filename))
  {
    if (!overwrite)
  ...
  if (!file.is_open())
  {
    ...
    return false;
     ...
     Eigen::IOFormat CleanFmt(4, 0, ", ", "\n", "[", "]");
  file << std::fixed;
  file << matrix.format(CleanFmt)<< std::endl;
file << "end of matrix" << std::endl;
  file.close();
