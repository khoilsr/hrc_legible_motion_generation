...
public:
  ...
  virtual void evaluate(const Eigen::MatrixXd& cost_vars, Eigen::VectorXd& costs) const 
  {
    ...
    virtual void evaluateHrc(const Eigen::MatrixXd& cost_vars1,const Eigen::MatrixXd& cost_vars2, Eigen::VectorXd& costs1,Eigen::VectorXd& costs2,int update,ros::NodeHandle node,ros::Publisher traj_pub) const
    {
      int n_task_pars = 0;
      Eigen::MatrixXd task_parameters(cost_vars1.rows(),n_task_pars);
      return evaluateHrc(cost_vars1,cost_vars2,task_parameters, costs1,costs2, update,node, traj_pub);
    };
  ...
  virtual void evaluate(const Eigen::MatrixXd& cost_vars, const Eigen::MatrixXd& task_parameters, Eigen::VectorXd& costs) const = 0;
    ...
    //new function
     virtual void evaluateHrc(const Eigen::MatrixXd& cost_vars1, const Eigen::MatrixXd& cost_vars2, const Eigen::MatrixXd& task_parameters, Eigen::VectorXd& costs1,Eigen::VectorXd& costs2,int update,ros::NodeHandle node,ros::Publisher traj_pub) const = 0;
  ...
  virtual bool savePerformRolloutsPlotScript(std::string directory) const
  {
    ...
    virtual bool savePerformRolloutsPlotScriptHrc(std::string directory) const
    {
      return true;
    }
