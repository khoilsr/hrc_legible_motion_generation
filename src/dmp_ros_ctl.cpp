...
int main(int n_args, char* args[])
{
    ...
    msg_.points[0].velocities.resize(7);
    msg_.points[0].accelerations.resize(7);
    ...
    MatrixXd cost_vars1, cost_vars2, cost_vars3, cost_vars4;        //variables relevant to computing the cost
    ...
    for (int i_update = 0; i_update < n_updates; i_update++) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            MatrixXd angularRollout(n_time_steps, 5);       //not used..
            MatrixXd rolloutFK(n_time_steps, 3);
            MatrixXd rolloutFKd(n_time_steps - 1, 3);       //not used in this function, gets declared anew in computeCosts..
            ...
            while ((i1 + i2 + i3) < 3 * n_samples) {
                ...
                if (a == 1 && i1 < n_samples) {     //ah -> to have different samples from each goal
                    ...
                    for (int hh = 0; hh < ts.size(); hh += sampleSize) {
                        ...
                        msg_.points[0].positions[6] = rollout(hh, 2) ;      //?? why are 6 and 2 switched??
        ...
        //performRollouts determines all the variables relevant to evaluating the cost function, saves values in cost_vars, needs samples generated by gaussian distribution
        ...
        for (int i = 0; i < 1; i++) {
            int n_samples = cost_vars1.rows();      //number of rows in cost_vars
            int n_dims = goal1.size();              //number of coefficients in goal
            int n_cost_vars = 4 * n_dims + 1; // y_1..y_D  yd_1..yd_D  ydd_1..ydd_D  t forcing_term_1..forcing_term_D
    ...
    VectorXd& goal, int n_samples)
{
    ...
    if (jerk_weight_ != 0.0) {
        ...
        for (int i = 0; i < ts.size() - 1; i++) {
            jerk_sum += abs(ydd(i + 1, 0) - ydd(i, 0)) / time_step;     //derivative
