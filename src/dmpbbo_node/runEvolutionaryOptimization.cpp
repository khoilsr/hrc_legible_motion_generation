...
void runEvolutionaryOptimizationHrc(
  const Task* const task,
  const TaskSolver* const task_solver1,
 const TaskSolver* const task_solver2,
  const DistributionGaussian* const initial_distribution1,   
 const DistributionGaussian* const initial_distribution2,
...
task_solver2->performRollouts(distribution2->mean().transpose(),cost_vars_eval2);
    ...
    task->evaluateHrc(cost_vars_eval1,cost_vars_eval2,cost_eval1,cost_eval2,(i_update+1)*100,node,traj_pub);
  ...
  task_solver2->performRollouts(samples2,cost_vars2);
    ...
    // 2B. Evaluate the samples
    task->evaluateHrc(cost_vars1,cost_vars2,costs1,costs2,i_update,node,traj_pub);
