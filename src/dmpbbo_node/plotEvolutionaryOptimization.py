...
directory1 = "/home/ndonia/Documents/Codes/Freek/src/dmp_bbo/demos/demoDmpBbo/Goal1"
directory2 = "/home/ndonia/Documents/Codes/Freek/src/dmp_bbo/demos/demoDmpBbo/Goal2"
...
def plotEvolutionaryOptimization(directory,axs,plot_all_rollouts=False):
    ...
    if ( (axs!=None) and (len(axs)>3) ):
        ...
        if (os.path.isfile(rollouts_python_script)):
            ...
            for update in range(n_updates):
                ...
                plt.setp(rollout_lines,color='black',linewidth=1)
                if (update==0):
                    plt.setp(rollout_lines,color='r',linewidth=2)
                if (update==n_updates-1):
                    plt.setp(rollout_lines,color='green',linewidth=2)
      
    if ( (axs!=None) and (len(axs)>4) ):
        ax = axs[4];
        rollouts_python_script = directory+'/plotRollouts2.py'
        if (os.path.isfile(rollouts_python_script)):
            lib_path = os.path.abspath(directory)
            sys.path.append(lib_path)
            from plotRollouts2 import plotRollouts2
            ...
            for update in range(n_updates):
                ...
                if plot_all_rollouts:
                    filename = "/cost_vars.txt"
                cost_vars = np.loadtxt(cur_directory+filename)
                rollout_lines = plotRollouts2(cost_vars,ax)
                color_val = (1.0*update/n_updates)
                #cur_color = [1.0-0.9*color_val,0.1+0.9*color_val,0.1]
                cur_color = [0.0-0.0*color_val, 0.0+1.0*color_val, 0.0-0.0*color_val]
                #print str(update)+" "+str(n_updates)+" "+str(cur_color) 
    ...
    if ( (axs!=None) and (len(axs)>5) ):
        ax = axs[5];
        rollouts_python_script = directory+'/plotRollouts3.py'
        if (os.path.isfile(rollouts_python_script)):
            lib_path = os.path.abspath(directory)
            sys.path.append(lib_path)
            from plotRollouts3 import plotRollouts3
            ...
            for update in range(n_updates):
                ...
                if plot_all_rollouts:
                    filename = "/cost_vars.txt"
                cost_vars = np.loadtxt(cur_directory+filename)
                rollout_lines = plotRollouts3(cost_vars,ax)
                color_val = (1.0*update/n_updates)
                #cur_color = [1.0-0.9*color_val,0.1+0.9*color_val,0.1]
                cur_color = [0.0-0.0*color_val, 0.0+1.0*color_val, 0.0-0.0*color_val]
                #print str(update)+" "+str(n_updates)+" "+str(cur_color) 
...
if __name__=='__main__':
    ...
    if (len(directories)==1):
        ...
        if (os.path.isfile(rollouts_python_script)):
            ...
            axs = [ fig.add_subplot(143), fig.add_subplot(144), fig.add_subplot(142) , fig.add_subplot(141),fig.add_subplot(145) , fig.add_subplot(146) ]
        else:
