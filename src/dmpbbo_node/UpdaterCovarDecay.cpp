...
void UpdaterCovarDecay::updateDistributionHrc(const DistributionGaussian& distribution, const MatrixXd& samples, const VectorXd& costs, VectorXd& weights, DistributionGaussian& distribution_new) const
{
  // Update the mean
  VectorXd mean_new;
  updateDistributionMeanHrc(distribution.mean(), samples, costs, weights, mean_new);
  distribution_new.set_mean(mean_new);
