...
{Eigen::IOFormat CleanFmt(4, 0, ", ", "\n", "[", "]");
  if (covar_decomposed_.size()==0)
  {
    ...
    covar_decomposed_ = A;
//cout << "covar_decomposed_: " << covar_decomposed_.format(CleanFmt) << endl;
...
{
//if(i_dim==6||i_dim==7||i_dim==8)
if(i_dim>=(n_dims/3)*2)
{double a=(*normal_distribution_unit)();
if(a>0)
z(i_dim)=0;
else z(i_dim)=(abs(a)*-1)/2.5;
//cout<<" z in 3rd dimension is "<< z(i_dim)<<endl;
}
...
//cout << "samples: " << samples.format(CleanFmt) << endl;
